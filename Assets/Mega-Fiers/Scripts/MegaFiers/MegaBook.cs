...
// added to smooth opening movement		
turn = 1.0f - Mathf.Cos( (float)turn * 1.57f );
turn = 1.0f - Mathf.Cos( (float)turn * 1.57f );
	...
	{
		double totalPageCount = (double)((pages.Count + 4) / 2);
		double page = ( ( bookalpha + 0.01f ) * totalPageCount ) / 100.0f;
		page -= 1.12f;
		
		if( (int)page < 0 )
			return( false );
		if( (int)page >= totalPageCount )
			return( false );
	...
	public bool IsValidPage( int page )
	{
		...
		if( (int)page < 0 )
			return( false );
		if( (int)page >= (int)totalPageCount )
			return( false );
	...
	public IEnumerator FlipToPage(int page)
	{
		pageTurning = true;
		double totalPageCount = (double)((pages.Count + 4) / 2);
		//Debug.Log(totalPageCount);
		...
		while ( (int)(bookalpha*1000.0f) != (int)((float)target*1000.0f) )
		{
			//Debug.Log( "" + bookalpha + " " + target );
			bookalpha = Mathf.MoveTowards(bookalpha, (float)target, keysensi * Time.deltaTime);
			yield return 0;
		}
