	...
	public void Position()
	{
		if ( path && rb && connected )
		{
			...
			if ( align )
			{
				...
				Vector3 arot = Quaternion.LookRotation(p1 - np).eulerAngles;

				rot.y = arot.y;
				rb.MoveRotation(Quaternion.Euler(rot));	//Quaternion.LookRotation(p1 - np));
			}
		}
	}
		...
		if ( path && rb && connected )
		{
			...
			if ( connected )
			{
				...
				if ( drive != 0.0f )
				{
					...
					//rb.AddForce(transform.forward * drive, ForceMode.Force);
				}
			}
		}
	}
