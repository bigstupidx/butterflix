	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		public int FindEntry(string name, bool ignoreCase)
		{
			...
			for (int i = 0; i < entries_.Length; i++) 
            {
                ...
                {
                    if (string.Compare(name, entries_[i].Name, StringComparison.Ordinal) == 0) 
                    {
					    return i;
                    }
                }
...
*/
		...
		int WriteCentralDirectoryHeader(ZipEntry entry)
		{
            ...
            byte[] rawComment = new byte[0]; // (entry.Comment != null) ? Encoding.ASCII.GetBytes(entry.Comment) : new byte[0];
