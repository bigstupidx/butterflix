...
public class FairyItemsManager 
{
	private static List<FairyItemData> 		itemList			= new List<FairyItemData>(64);
	private static bool						isItemListLoaded 	= false;
	
	//==================================================================
	
	public static FairyItemData GetFairyItem(eFairy fairy)
	{
		if(! isItemListLoaded)
		{
			LoadItemsList();
		...
		foreach(FairyItemData item in itemList)
		{
			if(fairy == item.fairy)
				return item;
		}
	...
	{		
		...
		case 4:
			...
			itemList[itemIndex].KeysRequired[ dataindex - 1 ] = int.Parse( costs[ 0 ] );
			itemList[itemIndex].GemsRequired[ dataindex - 1 ] = int.Parse( costs[ 1 ] );
			itemList[itemIndex].PopulationRequired[ dataindex - 1 ] = int.Parse( costs[ 2 ] );
			break;
		}
	}
