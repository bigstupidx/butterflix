...
public class JSON
{
	...
	public List< string > GetFieldsContaining(string fieldName)
	{
		List< string > ContainerList = new List< string >();
		
		foreach( KeyValuePair<string, object> message in fields )
		{
			if( message.Key.Contains( fieldName ) )
				ContainerList.Add( message.Key );
		}
	...
	{
		if (fields.ContainsKey(fieldName))
			return System.Convert.ToDouble(fields[fieldName]);
		else
			return 0.0;
	}
	...
	{
		return _JSON.SerializeObject(obj);
	}
	...
	public bool ContainsJSON(string fieldName)
	{
		if (fields.ContainsKey(fieldName))
			return( true );
		else
	...
	{
		IDictionary row = fields;
		
		return( row );
	}
	

	public IDictionary<string, object> ToIDictionaryStringObject()
	{
		IDictionary<string, object> row = fields;
